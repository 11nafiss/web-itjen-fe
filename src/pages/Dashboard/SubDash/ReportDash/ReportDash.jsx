...
const ReportDash = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getReportSearchAll({ take, page, keyword }));
    dispatch(getReportAllTake({ take, page }));
    dispatch(getReportAllCount());
  ...
  const handlePageChange = (event, value) => {
    ...
    dispatch(getReportAllTake({ take, page: value }));
    dispatch(getReportAllCount());
  };
  ...
  function handleSubmit(event) {
    ...
    dispatch(reportSearchAllSlice.actions.setSearchKeyword(searchInput));
    dispatch(getReportSearchAll({ take, page, keyword: searchInput }));
    dispatch(getReportSearchCount({ searchInput }));
    setSearchParams(updatedSearchParams.toString());

    if (searchInput === "" || searchInput == null) {
      navigate("/dashboard/laporan?page=1");
      dispatch(getReportAllCount());
    }
  }
    ...
    <Box component="main" sx={{ flexGrow: 1, p: 3, backgroundColor: "#D9D9D9", height: "100%" }}>
      <Grid container spacing={3}>
        <Grid item xs={12} md={12}>
          <CustomBox>
            <CustomTitle>Daftar Laporan</CustomTitle>
