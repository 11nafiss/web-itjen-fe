...
import { Button, IconButton, Input, FormControl, FormLabel, Textarea, Checkbox, FormHelperText, Select, Option } from "@mui/joy";
...
import { createReport, editReport } from "../../../../features/actions/report.action";
...
// Main Declaration
const CrReport = (props) => {
  const { id } = useParams();
  const [reportId, setReportId] = useState(id);
  console.log("ini id", reportId);
  ...
  const { errorMessage } = useAppSelector((state) => state.report.createReport);

  const fetchReportById = useCallback(async () => {
    const response = await laporanService.getLaporanById(id);
  ...
  const handleUploadReport = (e) => {
    ...
    if (props.mode === "Edit") {
      dispatch(editReport({ id, tableConfig }));
      ...
      dispatch(createReport(tableConfig));
    }
    ...
    <Box component="main" sx={{ flexGrow: 1, p: 3, backgroundColor: "#D9D9D9", height: "100%" }}>
      <Grid container spacing={3}>
        <Grid item xs={12} md={12}>
          <Kotak>
            <Judul>{props.mode} Laporan</Judul>
                    ...
                    <FormControl required sx={{ width: "100%" }}>
                      ...
                      <Box sx={{ width: "100%", display: "flex", flexDirection: "row", alignItems: "center" }}>
                        <Select
                          ...
                          required
                        >
                          <Option value="Kinerja" label="Kinerja">
                            Kinerja
                          </Option>
                          <Option value="Tahunan" label="Tahunan">
                            Tahunan
                          </Option>
                          <Option value="Layanan" label="Layanan">
                            Layanan
                          </Option>
                        </Select>
                      </Box>
