...
const create = (namaFile: string) => {
  return axios.post(urlImageGallery, JSON.stringify({
    namaFile: namaFile
  }),
  {
    headers: {
...
const edit = (id: any, namaFile: string) => {
  return axios.put(urlImageGallery + '/' + id, JSON.stringify({
    id: id,
    namaFile: namaFile
  }),
  {
...
const hapus = (id: any) => {
  return axios.delete(urlImageGallery + '/' + id, {
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
    }
...
const getImageGallery = (skip, take) => {
  return axios.get(urlImageGallery + '/' + skip + '/' + take, {
    headers: {
      'Content-Type': 'application/json'
    }
  }).then(res => {
...
const getJumlahImageGallery = () => {
  return axios.get(urlImageGallery + '/jumlah', {
    headers: {
      'Content-Type': 'application/json'
    }
  }).then(res => {
